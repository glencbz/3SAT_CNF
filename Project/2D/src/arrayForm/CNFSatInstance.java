...
public class CNFSatInstance
	{
		    ...
		    public int[][] computeOccurrenceMap(int[][] clauses, int numVars, int[] occurrenceNumMap){
		    	...
		    	int[][] occurMap = new int[clauses.length][numVars * 2];
		    	for (int i = 0; i < clauses.length; i++){
		    		for (int literal : clauses[i]){
		    			int pos = literal > 0 ? literal: numVars + Math.abs(literal);
		    			occurMap[occurrenceNumMap[pos-1]][pos - 1] = i + 1;
		    			occurrenceNumMap[pos-1]++;
		    		}
		    	}
		    	return occurMap;
		    }
			...
			public CnfSatInstance givenVarOccur(int[][] clauses, int var){
				...
				int[][] newOccurrenceMap = Arrays.copyOf(occurrenceMap, occurrenceMap.length);
		    	int[] newDeleted = Arrays.copyOf(deleted, deleted.length);
		    	
				
		    	int[] posOccurrences = occurrenceMap[position];
		    	int[] negOccurrences = occurrenceMap[position];
